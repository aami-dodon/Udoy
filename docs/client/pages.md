...
- `DashboardPage` is a protected view that greets the authenticated user, surfaces role-specific messaging, and links to account/admin utilities.
- `AccountPage` (in `features/account/pages`) allows authenticated users to update names, emails (with re-verification), and passwords.
